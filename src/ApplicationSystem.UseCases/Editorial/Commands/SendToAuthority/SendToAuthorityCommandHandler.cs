...
namespace ApplicationSystem.UseCases.Editorial.Commands.SendToAuthority
{
    /// <summary>
    ...
    internal class SendToAuthorityCommandHandler : IRequestHandler<SendToAuthorityCommand>
    {
        ...
        public async Task<Unit> Handle(SendToAuthorityCommand request, CancellationToken cancellationToken)
        {
            ...
            if (application == null)
            {
                throw new DomainException($"Application with Id:{request.ApplicationId} cannot be resent to authority.");
            }
